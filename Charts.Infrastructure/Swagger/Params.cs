...
namespace Charts.Api.Swagger
{
    public static class SwaggerParams
    {
        ...
        public static void AddPath(OpenApiOperation operation, OperationFilterContext context, string route, string key, string value)
        {
            var routeTemplate = context.ApiDescription.ActionDescriptor?.AttributeRouteInfo?.Template ?? string.Empty;
            ...
            foreach (var param in operation.Parameters)
            {
                // Path параметры находятся в ParameterLocation.Path
                if (param.Name.Equals(key, StringComparison.OrdinalIgnoreCase) && param.In == ParameterLocation.Path)
                {
                    param.Schema ??= new OpenApiSchema { Type = "string" };
                    param.Schema.Example = new OpenApiString(value);
                    param.Example = new OpenApiString(value);
                    param.Description ??= $"Пример: {value}";
                }
            }
        }
